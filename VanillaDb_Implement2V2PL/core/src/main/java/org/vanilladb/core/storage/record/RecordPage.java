...
public class RecordPage implements Record {
	...
	private Constant getVal(int offset, Type type) {
		...
		if(tx.privateSpace.containsKeyP(blk) && tx.privateSpace.getPrivateValue(blk).containsKey(offset)){
			
			return tx.privateSpace.getPrivateValue(blk).get(offset);
		}
	...
	private void setVal(int offset, Constant val) {
		...
		if (tx.privateSpace.containsKeyP(blk)) {
			HashMap <Integer,Constant> modi = tx.privateSpace.getPrivateValue(blk);
			modi.put(offset, val);
		}
		else{
			HashMap <Integer,Constant> modi = new HashMap <Integer,Constant>();
			modi.put(offset,val);
			tx.privateSpace.addInPrivate(blk, modi);
		}
		LogSeqNum lsn = doLog ? tx.recoveryMgr().logSetVal(currentBuff, offset, val)
				: null;
		if (lsn != null) {
			tx.privateSpace.saveLsn(blk,lsn);
		}
		//currentBuff.setVal(offset, val, tx.getTransactionNumber(), lsn); ->>move to Tx after getting C lock
	}
