...
import org.vanilladb.core.storage.tx.WorkSpaceForTx;
...
public class Transaction {
	...
	public WorkSpaceForTx privateSpace;
	...
	public Transaction(TransactionMgr txMgr, TransactionLifecycleListener concurMgr,
		...
		privateSpace = new WorkSpaceForTx();
	 ...
	 */
	public void commit() {
		for (Entry<BlockId, HashMap<Integer, Constant>> entry : this.privateSpace.getP().entrySet()) {
			this.concurMgr.getCLock(entry.getKey());
		}///gain c lock
		for (Entry<BlockId, HashMap<Integer,Constant>> entry : this.privateSpace.getP().entrySet()) {
			Buffer currentBuffer = this.bufferMgr.pin(entry.getKey());
			BlockId blk = entry.getKey();
			HashMap <Integer,Constant>modirecs = entry.getValue();
			for (Map.Entry<Integer, Constant> subentry : modirecs.entrySet()) {
				currentBuffer.setVal(subentry.getKey(), subentry.getValue(), txNum, this.privateSpace.getL().get(blk));
			}
			this.bufferMgr.unpin(currentBuffer);
			//currentBuffer.setVal(offset, val, txNum, lsn);
		}
		this.privateSpace.clearLsn();
		this.privateSpace.clearPrivate();
